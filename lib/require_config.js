...
function helper( baseDir ) {
   ...
   var api = {
      projectPath: function( requireRef ) {
         var absolutePath = requirejs.toUrl( requireRef );
         return path.relative( baseDir, absolutePath );
      },

      projectRef: function( requireRef ) {
         return path.join( 'laxar-application' , api.projectPath( requireRef ) );
      }
   };
   return api;
}
...
function requirejsFactory( file, options ) {
   ...
   var context = {
      ...
      requireConfig: fs.readFileSync( file )
   };
   ...
   config.baseUrl = path.relative( base || '.', config.baseUrl );
   config.deps = [];
