...
exports.create = function( requirejs, httpClient, widgetsRoot, pageLoader ) {
   'use strict';
      ...
      return findWidgetsFromFlow( pathToFlow )
         ...
         .then( function( widgetInfos ) {
            ...
            var promises = widgetInfos.map( function( widgetInfo ) {
               ...
               widgetPaths[ technology ].push( [ widgetsRootUrl, widgetInfo.dir, name ].join( '/' ) );
   ...
   function readControlJson( controlRef ) {
      var controlJsonPath = requirejs.toUrl( path.join( controlRef, 'control.json' ) );
      if( !( controlJsonPath in cachedPromises ) ) {
         cachedPromises[ controlJsonPath ] = httpClient.get( controlJsonPath )
            .then( function( response ) {
               return response.data;
            }, function() {
