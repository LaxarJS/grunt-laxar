...
exports.create = function( log, config ) {
   ...
   function collectWidgets( pages, themes ) {
      ...
      return q.all( pages.map( followPageToWidgets ) )
            ...
            return [ {
               ...
               references: {
                  ...
                  amd: {
                     ...
                     module: isAmdReference ? path.join( referenceInfo.ref, moduleName ) :
                        requirejsHelper.projectRef (path.join( 'laxar-path-widgets', widgetRef, moduleName ) )
                  }
               },
               controls: descriptor.controls || [],
               referencedAs: {
                  byUser: widgetRef,
                  derivedScheme: referenceInfo.scheme,
                  derivedReference: referenceInfo.ref
               }
            } ];
         } );
   ...
   function collectControls( widgets, themes ) {
      ...
      return q.all( widgets.map( followWidgetToControls ) )
      ...
      function followControl( controlRef ) {
         ...
         return readJson( descriptorPath )
            .then( identity, fakeDescriptor )
            .then( function( descriptor ) {
               var referenceInfo = extractScheme( controlRef, 'amd' );
               var isAmdReference = referenceInfo.scheme === 'amd';
               ...
               // Take into account possible theme-folder files
               themes.filter( nonDefault ).forEach( function( theme ) {
                  var basePart = [ '', theme.path, 'controls', nameFromDirectory ];
                  var themeCssPath = basePart.concat( cssPart ).join( path.sep );
                  toList.unshift( themeCssPath );
                  toWatch.unshift( themeCssPath );
               } );
               ...
               return [ {
                  ...
                  references: {
                     local: {
                        self: path.relative( requirejsHelper.projectPath( 'laxar-path-controls' ), controlPath )
                     },
                     amd: {
                        self: path.relative( requirejsHelper.projectPath( '.' ), controlPath ),
                        module: isAmdReference ? [ referenceInfo.ref, name ].join( URL_SEP ) :
                           requirejsHelper.projectRef( [ 'laxar-path-controls', referenceInfo.ref, name ].join( URL_SEP ) )
                     }
                  },
                  referencedAs: {
                     byUser: controlRef,
