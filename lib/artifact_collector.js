...
exports.create = function( log, config ) {
      ...
      function followWidget( widgetRef ) {
         ...
         return q.all( [
            ...
            return [ {
               ...
               references: {
                  file: {
                     reference: path.relative( requirejsHelper.projectPath( 'laxar-path-widgets' ), widgetPath )
                  },
                  amd: {
                     module: requirejsHelper.projectRef( [ 'laxar-path-widgets', widgetRef, name ].join( URL_SEP ) )
                  }
               },
   ...
   function collectThemes() {
      ...
      return readdir( themesRootPath )
         ...
         .then( function( dirs ) {
            return [ themeEntry( defaultThemePath, 'default.theme' ) ]
               .concat( dirs
                  .filter( function( dir ) {
                     return !!dir;
                  } )
   ...
   function resolveWidget( widgetRef ) {
      return requirejsHelper.projectPath( path.join( 'laxar-path-widgets', widgetRef ) );
   }

   function resolveControl( controlRef ) {
      return requirejsHelper.projectPath( controlRef );
   }
